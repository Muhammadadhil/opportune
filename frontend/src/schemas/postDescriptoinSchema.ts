...
// Define the schema for DescriptionData
 const DescriptionDataSchema = z.object({
     images: z.array(z.instanceof(File)).length(3, "Three image files are required").nonempty("At least one image file is required"), // Ensures there is at least one file in the array
